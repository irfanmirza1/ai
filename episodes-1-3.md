# Munchlings: Scripts for Episodes 1–3

**Key for every shot**

| Tag | Where the shot comes from |
|---|---|
| 🎬 **FLOW** | A Veo clip from Flow (Ingredients to Video). Paste the style line at the end. Always say "no talking, no voices". |
| 🗣️ **JOGG** | The character's talking photo in Jogg, with their fixed voice. One line per clip. |
| 📱 **REAL** | You filming with your phone: hands, the ingredients, the result. |
| 🎵 **SONG** | Kai's closing song. Put a kids' beat from the YouTube Audio Library under the Jogg Kai voice speaking the words in rhythm. |

**Rules for every recipe**
- All recipes are halal-friendly. Use halal-certified cheese.
- An adult always handles the oven, stove and knives.
- Put an allergy note (egg, dairy, gluten) on screen and in the description.

**Every episode follows the same shape**
1. Hook (under 10 seconds)
2. 5-second jingle
3. The problem
4. The adventure
5. The fact and word
6. The recipe (real footage)
7. The lesson
8. Kai's song
9. Goodbye

---

# EPISODE 1: "Broccoli Bob Wants a Superpower!"

| | |
|---|---|
| **Length** | about 4½ minutes |
| **Lesson** | Everyone is special in their own way. Be brave and try new foods. |
| **Fact** | The part of broccoli we eat is lots of tiny flower buds. |
| **Word** | **Floret** |
| **Recipe** | Broccoli Cheese Bites (broccoli, egg, cheese) |
| **Clips needed** | 11 FLOW, 17 JOGG, about 75s of REAL footage |
| **Credits** | about 330 (Lite tests + Fast finals) |

| # | Type | What we see | Line / sound |
|---|---|---|---|
| 1 | 🎬 FLOW | Close-up: Broccoli Bob on the kitchen counter with a paper-towel cape, jumping off a spoon and landing flat on his face. | Boing, then a soft "flop" sound |
| 2 | 🗣️ JOGG | Bob, lifting his head | **BOB:** "Today... I'm getting a SUPERPOWER!" |
| 3 | 🎵 | **Jingle:** the 5 characters wave. | "Munchlings!" |
| 4 | 🎬 FLOW | Bob walking slowly across the Garden Kitchen counter, kicking a pea. | Soft sad piano |
| 5 | 🗣️ JOGG | Bob | **BOB:** "Coco can find anything. Tim makes everyone laugh. Kai can sing." |
| 6 | 🗣️ JOGG | Bob | **BOB:** "But me? I'm just a tiny tree." |
| 7 | 🎬 FLOW | Carrot Coco hurrying in with her notebook. | Quick footsteps |
| 8 | 🗣️ JOGG | Coco | **COCO:** "Bob! Why the sad face?" |
| 9 | 🗣️ JOGG | Bob | **BOB:** "Everyone has a superpower. Except me." |
| 10 | 🗣️ JOGG | Coco, smiling | **COCO:** "Hmm... then let's find out!" |
| 11 | 🎬 FLOW | Coco and Bob walking into Fridge World, looking up in wonder. | Magical whoosh, adventure music |
| 12 | 🎬 FLOW | Tomato Tim rolls past, slips on a lettuce leaf, and pops up. | Rolling, then a boing |
| 13 | 🗣️ JOGG | Tim | **TIM:** "Oops-a-tomato! Did someone say SUPERPOWER?" |
| 14 | 🗣️ JOGG | Tim, proudly | **TIM:** "Mine is being super ROUND!" |
| 15 | 🎬 FLOW | Back in the Garden Kitchen: Potato Pete in his apron, stirring a pot. | Bubbling |
| 16 | 🗣️ JOGG | Pete | **PETE:** "Bob, did you know? Broccoli is made of tiny flower buds." |
| 17 | 🗣️ JOGG | Pete | **PETE:** "They're called... FLORETS!" *(show the word FLORET on screen)* |
| 18 | 🗣️ JOGG | Bob, amazed | **BOB:** "Flowers? On ME?" |
| 19 | 🗣️ JOGG | Pete | **PETE:** "And broccoli helps kids grow big and strong. That's a real superpower." |
| 20 | 🎬 FLOW | Magic bubbles float above the table showing a green soup, a pasta and a "tiny tree" salad. Bob's eyes sparkle. | Sparkle sounds |
| 21 | 🗣️ JOGG | Bob | **BOB:** "Soup... pasta... salad! Broccoli can be ALL of those?" |
| 22 | 🗣️ JOGG | Pete | **PETE:** "And today we'll make Broccoli Cheese Bites. Just THREE things!" |
| 23 | 📱 REAL | **Recipe (about 75s).** See below. | Pete's voice (Jogg audio only) |
| 24 | 🎬 FLOW | All 5 around a plate of golden bites, cheering. Bob stands proud in front. | Cheers |
| 25 | 🗣️ JOGG | Bob | **BOB:** "My superpower is helping kids grow strong! Tiny tree, big me!" |
| 26 | 🗣️ JOGG | Coco | **COCO:** "See? Everyone is special in their own way." |
| 27 | 🎬 FLOW + 🎵 | Kai dancing on the table while the others clap. | **SONG** (below) |
| 28 | 🗣️ JOGG | Tim, waving | **TIM:** "Try ONE new food this week! Bye-bye!" |
| 29 | End card | The group picture and "Next time: Tomato Tim's Big Oops!" | Jingle |

### Recipe for shot 23: Broccoli Cheese Bites

**Ingredients**
- 1 cup cooked, finely chopped broccoli
- 1 egg
- ½ cup grated cheese

**Shots and Pete's voice lines**

| # | What you film | Pete says |
|---|---|---|
| 1 | The 3 ingredients in a row, from above | "Number one: broccoli! Number two: one egg! Number three: cheese!" |
| 2 | Adult hands chopping the cooked broccoli | "A grown-up helps with the knife." |
| 3 | Cracking the egg, adding the cheese, a child's hand mixing | "Mix, mix, mix!" |
| 4 | Small spoonfuls on a lined tray | "Little tiny trees on the tray." |
| 5 | An adult putting the tray in the oven (180°C / 350°F, 15–18 minutes) | "Grown-ups use the oven. Now... we wait!" |
| 6 | Golden bites on a plate, a child's hand takes one | "Ta-da! Broccoli Cheese Bites!" |

**Kai's song**
> 🎵 Tiny tree, tiny tree, strong as strong can be!
> Soup or pasta, crunchy bite, broccoli makes me feel just right!
> Pop pop hooray!

### Upload details

- **Title:** Broccoli Bob Wants a Superpower! 🥦 | Munchlings Kids Cartoon + Easy Recipe
- **Thumbnail:** Bob in his paper cape, shocked face, with a plate of golden bites. Text: "MY SUPERPOWER?!"
- **Shorts to cut from this episode:**
  1. Tim's "super ROUND" slip, shots 12–14
  2. The recipe in 30 seconds
  3. The fact: "Broccoli is made of flowers?!" (shots 16–18)

---

# EPISODE 2: "Tomato Tim's Big Oops!"

| | |
|---|---|
| **Length** | about 4½ minutes |
| **Lesson** | Everyone makes mistakes. Say sorry and fix it together. |
| **Fact** | A tomato is really a **fruit**, because it has seeds inside. |
| **Word** | **Seed** |
| **Recipe** | Mini Pizza Toasts (bread, tomato sauce, cheese) |
| **Clips needed** | 12 FLOW, 16 JOGG, about 70s of REAL footage |

| # | Type | What we see | Line / sound |
|---|---|---|---|
| 1 | 🎬 FLOW | Coco has built a tall tower of sugar cubes on the counter. Tim rolls fast and CRASH, the tower falls. | Rolling, then a crash |
| 2 | 🗣️ JOGG | Tim, frozen | **TIM:** "Oops... a... tomato." |
| 3 | 🎵 | Jingle | "Munchlings!" |
| 4 | 🗣️ JOGG | Coco, upset | **COCO:** "Tim! That took me ALL morning!" |
| 5 | 🎬 FLOW | Tim rolls away quickly and disappears into Fridge World. | Whoosh, fridge door |
| 6 | 🗣️ JOGG | Bob, worried | **BOB:** "Where did Tim go?" |
| 7 | 🗣️ JOGG | Pete, calmly | **PETE:** "I think Tim feels bad. Let's go find him together." |
| 8 | 🎬 FLOW | Coco, Bob and Pete walk into Fridge World with a small flashlight. | Adventure music |
| 9 | 🎬 FLOW | They search between jars and milk bottles. | Echoing footsteps |
| 10 | 🗣️ JOGG | Coco, reading her notebook | **COCO:** "Clue! Little red footprints... going behind the milk!" |
| 11 | 🎬 FLOW | Tim sitting alone behind the milk bottle, looking sad. | Soft music |
| 12 | 🗣️ JOGG | Tim | **TIM:** "I'm sorry, Coco. I broke your tower. I'm just a silly vegetable." |
| 13 | 🗣️ JOGG | Pete | **PETE:** "Did you know, Tim? You're not a vegetable at all!" |
| 14 | 🗣️ JOGG | Tim, shocked | **TIM:** "WHAT?!" |
| 15 | 🗣️ JOGG | Pete | **PETE:** "A tomato has SEEDS inside, so it's really a fruit!" *(show the word SEED on screen)* |
| 16 | 📱 REAL | A real tomato cut in half, close-up of the seeds. | Sparkle sound |
| 17 | 🗣️ JOGG | Coco, smiling | **COCO:** "Thank you for saying sorry, Tim. Everyone makes mistakes." |
| 18 | 🗣️ JOGG | Coco | **COCO:** "Let's fix it... and make something yummy together!" |
| 19 | 🎬 FLOW | The crew walks back into the Garden Kitchen, Tim in the middle, happy. | Happy music |
| 20 | 🗣️ JOGG | Pete | **PETE:** "Today: Mini Pizza Toasts! Just THREE things!" |
| 21 | 📱 REAL | **Recipe (about 70s).** See below. | Pete and Tim's voices (Jogg audio) |
| 22 | 🎬 FLOW | All 5 rebuilding the sugar tower together, laughing. | Cheers |
| 23 | 🗣️ JOGG | Tim | **TIM:** "When I make a mistake, I say sorry and help fix it!" |
| 24 | 🎬 FLOW + 🎵 | Kai dancing. | **SONG** |
| 25 | 🗣️ JOGG | Tim | **TIM:** "Oops-a-tomato... it's okay! Bye-bye!" |
| 26 | End card | "Next time: Food Passport: PAKISTAN!" | Jingle |

### Recipe for shot 21: Mini Pizza Toasts

**Ingredients**
- 2 slices of bread
- 2 tbsp tomato sauce
- ½ cup grated cheese

**Shots and voice lines**

| # | What you film | Voice line |
|---|---|---|
| 1 | The 3 ingredients from above | **Pete:** "Bread! Tomato sauce! Cheese!" |
| 2 | An adult cutting the bread into small squares | (no line) |
| 3 | A child's hand spreading the sauce | **Tim:** "Hey, that's my cousin!" (a joke) |
| 4 | Sprinkling the cheese | (no line) |
| 5 | An adult puts them in the oven (200°C / 400°F, 6–8 minutes) until the cheese melts | **Pete:** "A grown-up uses the oven." |
| 6 | The finished toasts on a plate | **Pete:** "Ta-da! Mini Pizza Toasts!" |

**Kai's song**
> 🎵 Oops, oops, that's okay, say you're sorry, fix it, play!
> Tomato's red and full of seeds, a fruit that's friendly, yes indeed!
> Pop pop hooray!

### Upload details

- **Title:** Tomato Tim's Big Oops! 🍅 | Is a Tomato a FRUIT? | Munchlings Kids Cartoon
- **Thumbnail:** Tim with a shocked face in front of a falling tower. Text: "I'M A FRUIT?!"
- **Shorts to cut from this episode:**
  1. The tower crash, shots 1–2
  2. The "Tomato is a FRUIT?!" fact
  3. The pizza toast recipe

---

# EPISODE 3: "Food Passport: PAKISTAN! 🇵🇰"

| | |
|---|---|
| **Length** | about 5 minutes |
| **Lesson** | Food from around the world is exciting. Try something new. |
| **Fact** | A paratha is flaky because the dough is folded into **layers**. |
| **Word** | **Mazedaar!** (Urdu for "delicious") |
| **Recipe** | Simple Paratha (flour, water, ghee or butter, plus a pinch of salt) |
| **Clips needed** | 13 FLOW, 15 JOGG, about 80s of REAL footage |

**New set needed:** make this once in Flow.
```
Colorful Pakistani street market in the afternoon: small food stalls, a hot round tawa pan on a stall, bright truck-art style decorations with flowers and patterns, hanging lights, friendly and cheerful, no characters, wide shot.
[STYLE LOCK]
```

| # | Type | What we see | Line / sound |
|---|---|---|---|
| 1 | 🎬 FLOW | The Magic Lunchbox on the counter suddenly glows and shakes. | Magic hum, sparkles |
| 2 | 🗣️ JOGG | Coco, excited | **COCO:** "The Magic Lunchbox is glowing! That means... FOOD PASSPORT!" |
| 3 | 🎵 | Jingle | "Munchlings!" |
| 4 | 🎬 FLOW | A paper clue floats out of the lunchbox into Coco's hands. | Paper flutter |
| 5 | 🗣️ JOGG | Coco, reading | **COCO:** "Round and flat, soft and flaky, cooked on a hot pan..." |
| 6 | 🗣️ JOGG | Tim | **TIM:** "A pancake? A pizza? A frisbee?!" |
| 7 | 🗣️ JOGG | Pete, smiling | **PETE:** "Let's go and find out!" |
| 8 | 🎬 FLOW | All 5 jump into the glowing lunchbox, one by one. | Whoosh and a "pop pop" |
| 9 | 🎬 FLOW | The crew lands in the colorful Pakistani street market and looks around amazed. | Happy music, market sounds |
| 10 | 🗣️ JOGG | Kai | **KAI:** "Look at all the colors! Where ARE we?" |
| 11 | 🗣️ JOGG | Coco, with her notebook | **COCO:** "We're in... PAKISTAN!" *(a flag appears)* |
| 12 | 🎬 FLOW | Close-up of the stall: a round golden flatbread cooking on the hot tawa. Steam rises. | Sizzle |
| 13 | 🗣️ JOGG | Pete | **PETE:** "That's a PARATHA! People love it for breakfast." |
| 14 | 🗣️ JOGG | Pete | **PETE:** "Did you know? It's flaky because the dough is folded into LAYERS." *(show the word LAYERS)* |
| 15 | 🗣️ JOGG | Bob | **BOB:** "Layers? Like a sandwich... but made of bread!" |
| 16 | 🗣️ JOGG | Coco | **COCO:** "And here, when food is yummy, you say... MAZEDAAR!" *(show the word MAZEDAAR)* |
| 17 | 🗣️ JOGG | Tim, jumping | **TIM:** "Mazedaar! Mazedaar!" |
| 18 | 🎬 FLOW | The crew jumps back through a glowing portal into the Garden Kitchen. | Whoosh |
| 19 | 🗣️ JOGG | Pete | **PETE:** "Let's make our own paratha at home!" |
| 20 | 📱 REAL | **Recipe (about 80s).** See below. | Pete's voice |
| 21 | 🎬 FLOW | All 5 around a plate of golden paratha, cheering. | Cheers |
| 22 | 🗣️ JOGG | Bob | **BOB:** "Trying food from another country is like an adventure!" |
| 23 | 🎬 FLOW + 🎵 | Kai dancing on a table in the market set. | **SONG** |
| 24 | 🗣️ JOGG | Coco | **COCO:** "Where should the Magic Lunchbox take us next? Bye-bye!" |
| 25 | End card | The group picture and "Next Food Passport: coming soon!" | Jingle |

### Recipe for shot 20: Simple Paratha

**Ingredients**
- 1 cup wheat flour (atta) with a pinch of salt
- about ⅓ cup water
- ghee or butter

**Shots and Pete's voice lines**

| # | What you film | Pete says |
|---|---|---|
| 1 | The 3 ingredients from above | "Flour! Water! Ghee!" |
| 2 | A child's hands mixing the flour and water into a soft dough | "Squish, squish... soft dough!" |
| 3 | Rolling a ball flat, spreading ghee, folding it over and over, rolling again | "Fold it... fold it again... that makes LAYERS!" |
| 4 | **An adult only**, cooking on a hot tawa or pan with a little ghee, both sides | "The pan is very hot. Only grown-ups cook it!" |
| 5 | A golden paratha on a plate, a child tearing a piece | "Ta-da! Paratha! Mazedaar!" |

**Kai's song**
> 🎵 Round and flaky, hot and new, paratha made for me and you!
> Fold the layers, one, two, three, mazedaar, yes, yummy for me!
> Pop pop hooray!

### Upload details

- **Title:** Food Passport: PAKISTAN! 🇵🇰 | What is Paratha? | Munchlings Kids Cartoon
- **Thumbnail:** The crew in the colorful market, with a big golden paratha. Text: "MAZEDAAR!"
- **Shorts to cut from this episode:**
  1. The jump into the lunchbox and the arrival in Pakistan
  2. The paratha folding (layers) up close
  3. "Say MAZEDAAR!"

---

## Description template (use it for every episode)

```
Join the Munchlings! 🥦🥕🍅🥔🌽 In today's episode, [one sentence story].

🧠 Today's fact: [fact]
📖 Today's word: [word]
🍳 Today's recipe: [recipe name] (3 ingredients!)

👨‍👩‍👧 Parents: free printable recipe card + colouring page 👉 [website link]
⚠️ Always cook with a grown-up. Contains: [egg / dairy / gluten].

Animation made with AI tools; story, recipe and voices directed by the Munchlings team.
```

## Publish checklist

- [ ] Set the audience to **"Yes, it's made for kids"** for the episodes. The parent recipe clips are **not** made for kids.
- [ ] Turn on auto-dubbing (in Studio, under Languages).
- [ ] Add each episode to a **playlist**: "Munchlings: All Episodes".
- [ ] Post Episodes 1, 2 and 3 a week apart. Post 3 Shorts in between each one.
